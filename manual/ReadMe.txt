...
Запуск программ:
----------------
Программы не имеют графического интерфейса. Для их запуска необходимо в терминале перейти в директорию программы и выполнить команду python3 Program.py

Port_Status собирает данные независимо от других частей программы.
Перед выполнением запросов к базе данных без учета количества сессий необходимо сформировать таблицу с данными об абонентах, т.е. дать отработать Make_Abon_Dsl
Перед выполнением запросов к базе данных с учетом количества сессий необходимо запустить Make_Abon_Onyma (для формирования таблицы с данными Онимы), а затем Session_Count для запуска проверки сессий.
